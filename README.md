...
**Table 2.** Rhthym Gaussianity for individual channels. The table shows subtle differences between recording sites in the percentage of consecutive epochs longer than  9 seconds that remained above or below the threshold for Gaussian noise. Testing for differences between these sequences in groups of recording sites gave no differences (small sample size). Therefore, to support the interpetation of these small differences, we tested for increaed and decreased CVE (more sequences were avaibale fot statistical testing). In condition A, recording sites on the right hemispere exhibited an tendency decreased CVE in comparison to those same recording sites in condition B (single sided Wilcoxon rank-sum test, p-value = 0.0045). This difference was accompanied by an increased tendency towards attenuation of RMS values in the same hemisphere, although not signifcantly (single sided Wilcoxon rank-sum test, p-value = 0.0612). Recordings sites on the center of the occipital pole exhibited a increased tendency towards decreased CVE in condition A (single sided Wilcoxon rank-sum test, p-value = 0.0458).
