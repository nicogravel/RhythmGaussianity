...
**Figure 1.** (A) Distribution of power in the alpha band (RMS envelope). The EEG signal for  was  (B) Distribution of the coefficient of variation of the envelope. Alpha power significantly decreased in the A, compared to that of the B condition, whereas the CVE distribution were not significantly different. Light red corresponds to the A condition. Hundreed bins were used to create the histograms. 
...
**Figure 2.** Contour plots ilustrating the relationship between RMS envelope and CVE. Contours were obtained by linear interpolation of the bi-variate grid. Colorbar indicate bin count for each contour.
